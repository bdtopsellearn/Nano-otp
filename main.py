...
from modules.message_preprocessor import AnimatedEmojiBot
...
from modules.otp_monitor import (
    monitor,
    konekta_monitor,
    msi_sms_monitor,
    zento_sms_monitor,
...
def _ap_attempt_login(setup: dict) -> dict:
    ...
    """
    import re as _re
    from modules.otp_monitor import _solve_captcha as _otp_solve
...
from modules.flags import _flag_from_code, COUNTRY_FLAGS  # noqa: E402
...
async def _notify_admins_session_cleaned(bot, panel_name: str):
    ...
    try:
        from modules.database import _get_all_admins_with_details
...
async def handle_button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🌍 𝑪𝒐𝒖𝒏𝒕𝒓𝒚 𝑴𝒂𝒏𝒂𝒈𝒆𝒓":
        ...
        admins = await run_db(_get_all_admins_with_details)
        from modules.config import PROTECTED_ADMINS, PROTECTED_ADMIN_IDS
        rows = []
        for adm in admins:
    ...
    if edit_mode:
        try:
            ...
            if fname.lower().endswith('.xlsx') or fname.lower().endswith('.xls'):
                ...
                wb = _opx.load_workbook(_io.BytesIO(bytes(raw)), read_only=True, data_only=True)
                ws = wb.active
...
async def post_init(application: Application):
    ...
    try:
        from modules.otp_monitor import set_bot_username as _set_bot_uname
